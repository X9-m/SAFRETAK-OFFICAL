  ...
  return <main className="role-auth-page" dir="rtl">
    ...
    <section className="auth-card role-auth-card screen-enter">
      ...
      {step === 'phone' ? <form className="form-area" onSubmit={requestCode}>
        ...
        <button className="primary-button" type="submit" disabled={busy || phone.length < 9}>{busy ? <Loader2 className="spin" size={17} /> : <LogIn size={17} />}إرسال رمز الدخول</button>
        ...
        {error ? <div className="login-alert" role="alert">{error}</div> : null}
        <button className="primary-button" type="submit" disabled={busy || code.length !== 6 || seconds <= 0}>{busy ? <Loader2 className="spin" size={17} /> : <CheckCircle2 size={17} />}تأكيد الدخول</button>
...
function ServiceEditor({ draft, onChange, onClose, onSave, busy }: { draft: ServiceDraft; onChange: (draft: ServiceDraft) => void; onClose: () => void; onSave: () => void; busy: boolean }) {
  return <div className="role-modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="role-dialog role-dialog-wide" role="dialog" aria-modal="true">
    ...
    <footer><button className="secondary-button" type="button" onClick={onClose}>إلغاء</button><button className="primary-button compact" type="button" onClick={onSave} disabled={busy || draft.title.trim().length < 2 || !draft.price}>{busy ? <Loader2 className="spin" size={16} /> : <Save size={16} />}حفظ الخدمة</button></footer>
  </section></div>;
}
...
function OfficePortal({ activeTab: tab, snapshot, reload, runAction }: { activeTab: OfficeTab; snapshot: RolePortalSnapshot; reload: () => Promise<void>; runAction: (action: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  const renderServices = () => {
    ...
    return <><Toolbar title="الخدمات والرحلات" description="إضافة وتعديل ونشر كل عروض المكتب." search={search} onSearch={setSearch} action={<button className="primary-button compact" type="button" onClick={() => editService()}><Plus size={16} />إضافة خدمة</button>} />
...
function AdminPortal({ activeTab: tab, snapshot, reload, runAction }: { activeTab: AdminTab; snapshot: RolePortalSnapshot; reload: () => Promise<void>; runAction: (action: () => Promise<unknown>, success: string) => Promise<void> }) {
  ...
  const officesPage = <><Toolbar title="إدارة المكاتب" description="إنشاء واعتماد وإيقاف المكاتب وخطط الاشتراك." search={search} onSearch={setSearch} action={<button className="primary-button compact" type="button" onClick={()=>setOfficeDraft({subscription_plan:'Free',is_approved:false})}><Plus size={16} />مكتب جديد</button>} />{officeDraft ? <section className="role-card inline-editor"><h2>إضافة مكتب سياحي</h2><div className="role-form-grid"><label><span>اسم المكتب</span><input value={text(officeDraft,'name')} onChange={(e)=>setOfficeDraft({...officeDraft,name:e.target.value})} /></label><label><span>رقم هاتف الدخول</span><input dir="ltr" value={text(officeDraft,'phone')} onChange={(e)=>setOfficeDraft({...officeDraft,phone:e.target.value})} /></label><label><span>الموقع</span><input value={text(officeDraft,'location')} onChange={(e)=>setOfficeDraft({...officeDraft,location:e.target.value})} /></label><label><span>رقم الترخيص</span><input value={text(officeDraft,'license_number')} onChange={(e)=>setOfficeDraft({...officeDraft,license_number:e.target.value})} /></label><label><span>الخطة</span><select value={text(officeDraft,'subscription_plan','Free')} onChange={(e)=>setOfficeDraft({...officeDraft,subscription_plan:e.target.value})}><option>Free</option><option>Basic</option><option>Premium</option></select></label><label className="role-check"><input type="checkbox" checked={bool(officeDraft,'is_approved')} onChange={(e)=>setOfficeDraft({...officeDraft,is_approved:e.target.checked})} /><span>اعتماد مباشرة</span></label></div><footer><button className="secondary-button compact" type="button" onClick={()=>setOfficeDraft(null)}>إلغاء</button><button className="primary-button compact" type="button" onClick={()=>void runAction(async()=>{await rolePortalClient.admin.createOffice(officeDraft);setOfficeDraft(null);},'تم إنشاء المكتب وحساب الدخول.')}><Save size={15}/>إنشاء</button></footer></section>:null}<div className="role-card role-table-card"><div className="role-table"><div className="role-tr role-th"><span>المكتب</span><span>الترخيص</span><span>الخطة</span><span>الاعتماد</span><span>التفعيل</span><span>حفظ</span></div>{filtered(offices,['name','location','license_number','subscription_plan']).map((row)=><AdminOfficeRow key={idOf(row)} row={row} runAction={runAction}/>)}</div></div></>;
...
export function RoleApp({ expectedRole }: { expectedRole: ExpectedRole }) {
  ...
  if(!snapshot)return <main className="loading-screen connection-screen" dir="rtl"><CircleAlert size={38}/><h1>تعذر تحميل البوابة</h1><p>{error}</p><button className="primary-button compact" type="button" onClick={()=>void bootstrap()}><RefreshCcw size={17}/>إعادة المحاولة</button></main>;
