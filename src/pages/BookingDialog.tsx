import { ArrowLeft, ArrowRight, Briefcase, CalendarDays, Check, CheckCircle2, Coffee, IdCard, Loader2, Minus, Plane, Plus, ShieldAlert, TicketCheck, UserRound, UsersRound, X } from 'lucide-react';
...
type FareClass = 'light' | 'standard' | 'flex';
type MealType = 'standard' | 'vegan' | 'premium';
...
const occupiedSeats = new Set(['1A', '1C', '2D', '2F', '3B', '5A', '7C', '7D', '8F', '10A', '10B']);
const seatRows = Array.from({ length: 12 }, (_, index) => index + 1);
const seatColumns = ['A', 'B', 'C', 'D', 'E', 'F'];
const passengerLabels: Record<FlightPassengerType, string> = { adult: 'بالغ', child: 'طفل', infant: 'رضيع' };
const farePrices: Record<FareClass, number> = { light: 0, standard: 30, flex: 65 };
const mealPrices: Record<MealType, number> = { standard: 0, vegan: 0, premium: 18 };
const seatPrice = (seat: string): number => { const row = Number.parseInt(seat, 10); return row <= 3 ? 20 : row <= 10 ? 10 : 5; };
...
export function BookingDialog({ service, onClose, onSubmit, onViewBookings, travelerName }: BookingDialogProps) {
  ...
  const [meal, setMeal] = useState<MealType>('standard');
  ...
  const flightQuantity = flightAdults + flightChildren + flightInfants;
  const seatPassengerCount = flightAdults + flightChildren;
  const bookingQuantity = service.type === 'flight' ? flightQuantity : quantity;
  const manifestCount = travelerManifestCount(service.type, bookingQuantity);
  const manifestTravelDate = primaryDate;
  const detailText = (key: string): string => typeof service.details[key] === 'string' ? String(service.details[key]) : '';
  const flightOrigin = detailText('origin') || detailText('departure') || 'عمّان';
  const flightDestination = detailText('destination') || service.location || 'الوجهة';
  const transitCity = detailText('transit_city') || detailText('layover_city');

  const flightPricingFactor = flightAdults + flightChildren * 0.75 + flightInfants * 0.15;
  const flightUpgradeTotal = service.type === 'flight'
    ? (farePrices[fareClass] + mealPrices[meal]) * seatPassengerCount + selectedSeats.reduce((sum, seat) => sum + seatPrice(seat), 0)
    : 0;
  const pricingUnits = useMemo(() => {
    ...
    if (service.type === 'flight') return flightPricingFactor;
    return quantity;
  }, [endDate, flightPricingFactor, primaryDate, quantity, rooms, service.type]);
  ...
  const estimate = useMemo(() => (service.price + (option?.priceDelta || 0)) * pricingUnits + selectedAddOns.reduce((sum, item) => sum + item.price * bookingQuantity, 0) + flightUpgradeTotal, [bookingQuantity, flightUpgradeTotal, option?.priceDelta, pricingUnits, selectedAddOns, service.price]);
  ...
  useEffect(() => {
    if (service.type !== 'flight') return;
    setTravelers((current) => current.map((traveler, index) => ({
      ...traveler,
      passengerType: index < flightAdults ? 'adult' : index < flightAdults + flightChildren ? 'child' : 'infant',
      documentType: 'passport',
    })));
    setSelectedSeats((current) => current.slice(0, seatPassengerCount));
  ...
  const validateDetails = (): string | null => {
    ...
    if (!Number.isInteger(bookingQuantity) || bookingQuantity < 1 || bookingQuantity > maxQuantity) return 'العدد المطلوب غير متاح.';
    ...
    if (service.type === 'flight') {
      if (flightAdults < 1 || flightChildren < 0 || flightInfants < 0 || flightInfants > flightAdults) return 'راجع أعداد البالغين والأطفال والرضّع.';
      if (tripType === 'round_trip' && (!isBookableDate(endDate, today) || endDate <= primaryDate)) return 'اختر تاريخ عودة بعد تاريخ المغادرة.';
      if (new Set(selectedSeats).size !== selectedSeats.length || selectedSeats.length > seatPassengerCount || selectedSeats.some((seat) => !/^([1-9]|1[0-2])[A-F]$/.test(seat) || occupiedSeats.has(seat))) return 'راجع المقاعد المختارة.';
    }
    return null;
  };
  ...
  const changeAdults = (delta: number) => {
    const next = Math.max(1, Math.min(9, flightAdults + delta));
    if (delta > 0 && flightQuantity >= maxQuantity) return;
    setFlightAdults(next);
    setFlightInfants((current) => Math.min(current, next));
  };
  const changeChildren = (delta: number) => {
    if (delta > 0 && flightQuantity >= maxQuantity) return;
    setFlightChildren((current) => Math.max(0, Math.min(9, current + delta)));
  };
  const changeInfants = (delta: number) => {
    if (delta > 0 && (flightQuantity >= maxQuantity || flightInfants >= flightAdults)) return;
    setFlightInfants((current) => Math.max(0, Math.min(flightAdults, current + delta)));
  };
  const toggleSeat = (seat: string) => setSelectedSeats((current) => current.includes(seat)
    ? current.filter((item) => item !== seat)
    : current.length < seatPassengerCount ? [...current, seat] : current);

  const buildDetails = (): Record<string, unknown> => {
    const common = {
      quantity: bookingQuantity,
      notes: cleanMultilineText(notes, 500),
      option_id: optionId || undefined,
      add_on_ids: addOnIds,
      travelers: manifestRequired ? sanitizeTravelerManifest(travelers).map((traveler) => ({
        full_name: traveler.fullName,
        nationality: traveler.nationality,
        document_type: traveler.documentType,
        document_number: traveler.documentNumber,
        document_expiry: traveler.documentExpiry,
        passenger_type: traveler.passengerType,
    ...
    if (service.type === 'flight') return {
      ...common,
      outbound_date: primaryDate,
      return_date: tripType === 'round_trip' ? endDate : null,
      trip_type: tripType,
      cabin_class: cabinClass,
      adult_count: flightAdults,
      child_count: flightChildren,
      infant_count: flightInfants,
  ...
  return (
    <div className="modal-backdrop modal-enter" onMouseDown={(event) => event.target === event.currentTarget && !submitting && onClose()}>
      <section ref={dialogRef} className="booking-dialog full-booking-dialog dialog-enter" role="dialog" aria-modal="true" aria-labelledby="booking-dialog-title" tabIndex={-1}>
        ...
        {created ? <div className="booking-success" aria-live="polite"><CheckCircle2 size={52} /><h3>تم إرسال طلب الحجز</h3><p>رقم الحجز</p><strong>{created.referenceCode}</strong>{service.type === 'flight' ? <div className="flight-success-ticket"><div><Plane size={18} /><span>{flightOrigin}</span><ArrowLeft size={16} /><span>{flightDestination}</span></div><small>{formatDate(primaryDate)} · {passengerLabels.adult}: {flightAdults} · {passengerLabels.child}: {flightChildren} · {passengerLabels.infant}: {flightInfants}</small><small>الفئة: {fareClass === 'light' ? 'Light' : fareClass === 'standard' ? 'Standard' : 'Flex'} · المقاعد: {selectedSeats.length ? selectedSeats.join('، ') : 'تحدد لاحقًا'}</small></div> : null}<small>الطلب بانتظار مراجعة المكتب. حالة الدفع تبقى غير مدفوعة إلى أن يؤكدها المكتب أو مزود الدفع.</small><div className="booking-success-actions"><button type="button" className="primary-button" onClick={() => onViewBookings(created)}>عرض الحجز</button><button type="button" className="secondary-button" onClick={onClose}>العودة للخدمة</button></div></div> : <>
          ...
          <div className="dialog-form booking-step-content">
            {step === 'details' ? <div className="booking-form-grid screen-enter">
              ...
              {service.type === 'flight' ? <><label className="field-group"><span>نوع الرحلة</span><select value={tripType} onChange={(event) => setTripType(event.target.value)}><option value="one_way">ذهاب فقط</option><option value="round_trip">ذهاب وعودة</option></select></label><label className="field-group"><span>درجة المقصورة</span><select value={cabinClass} onChange={(event) => setCabinClass(event.target.value)}><option value="economy">السياحية</option><option value="business">رجال الأعمال</option><option value="first">الأولى</option></select></label><div className="flight-passenger-counts"><div className="flight-passenger-count"><div><strong>البالغون</strong><small>12 سنة فأكثر</small></div><div className="quantity-control"><button type="button" onClick={() => changeAdults(-1)} disabled={flightAdults <= 1}><Minus size={15} /></button><strong>{flightAdults}</strong><button type="button" onClick={() => changeAdults(1)} disabled={flightQuantity >= maxQuantity}><Plus size={15} /></button></div></div><div className="flight-passenger-count"><div><strong>الأطفال</strong><small>من سنتين إلى 11 سنة</small></div><div className="quantity-control"><button type="button" onClick={() => changeChildren(-1)} disabled={flightChildren <= 0}><Minus size={15} /></button><strong>{flightChildren}</strong><button type="button" onClick={() => changeChildren(1)} disabled={flightQuantity >= maxQuantity}><Plus size={15} /></button></div></div><div className="flight-passenger-count"><div><strong>الرضّع</strong><small>أقل من سنتين</small></div><div className="quantity-control"><button type="button" onClick={() => changeInfants(-1)} disabled={flightInfants <= 0}><Minus size={15} /></button><strong>{flightInfants}</strong><button type="button" onClick={() => changeInfants(1)} disabled={flightQuantity >= maxQuantity || flightInfants >= flightAdults}><Plus size={15} /></button></div></div></div>{transitCity ? <div className="availability-warning full-span"><Plane size={17} />توقف ترانزيت في {transitCity}{detailText('layover_duration') ? ` لمدة ${detailText('layover_duration')}` : ''}</div> : null}</> : null}
            ...
            {step === 'travelers' ? <div className="traveler-manifest screen-enter">
              ...
              <div className="traveler-list">{travelers.map((traveler, index) => <section className="traveler-card" key={index}>
                <header><span><UserRound size={17} />{service.type === 'car' ? 'السائق' : `المسافر ${index + 1}`}{service.type === 'flight' ? <em className="passenger-type-tag"> · {passengerLabels[traveler.passengerType]}</em> : null}</span>{index === 0 ? <small>صاحب الحجز</small> : null}</header>
                <div className="booking-form-grid">
                  ...
                  {service.type === 'flight' ? <label className="field-group"><span>تاريخ الميلاد</span><input type="date" max={primaryDate} value={traveler.birthDate} onChange={(event) => updateTraveler(index, { birthDate: event.target.value })} /></label> : null}
            ...
            {step === 'options' ? <div className="booking-options screen-enter">
              {service.type === 'flight' ? <div className="flight-upgrades"><section><h3><Briefcase size={17} /> فئة التذكرة والأمتعة</h3><div className="flight-fare-grid"><button type="button" className={`flight-choice ${fareClass === 'light' ? 'active' : ''}`} onClick={() => setFareClass('light')}><strong>Economy Light</strong><span>حقيبة مقصورة 7 كجم، بدون حقيبة شحن.</span><b>مشمول</b></button><button type="button" className={`flight-choice ${fareClass === 'standard' ? 'active' : ''}`} onClick={() => setFareClass('standard')}><strong>Economy Standard</strong><span>حقيبة مقصورة + حقيبة شحن 23 كجم.</span><b>+ {formatMoney(farePrices.standard)} لكل مقعد</b></button><button type="button" className={`flight-choice ${fareClass === 'flex' ? 'active' : ''}`} onClick={() => setFareClass('flex')}><strong>Economy Flex</strong><span>حقيبتا شحن ومرونة أكبر في التعديل والإلغاء.</span><b>+ {formatMoney(farePrices.flex)} لكل مقعد</b></button></div></section><section><h3><Coffee size={17} /> الوجبة</h3><div className="flight-meal-grid"><button type="button" className={`flight-choice ${meal === 'standard' ? 'active' : ''}`} onClick={() => setMeal('standard')}><strong>قياسية</strong><span>الوجبة الأساسية المتاحة على الرحلة.</span><b>مشمولة</b></button><button type="button" className={`flight-choice ${meal === 'vegan' ? 'active' : ''}`} onClick={() => setMeal('vegan')}><strong>نباتية</strong><span>طلب وجبة نباتية مسبقًا.</span><b>بدون فرق سعر</b></button><button type="button" className={`flight-choice ${meal === 'premium' ? 'active' : ''}`} onClick={() => setMeal('premium')}><strong>فاخرة</strong><span>وجبة مطورة حسب توفر شركة الطيران.</span><b>+ {formatMoney(mealPrices.premium)} لكل مقعد</b></button></div></section><section className="flight-seat-section"><div className="flight-seat-heading"><div><h4>اختيار المقاعد</h4><p>اختياري، ويمكن للمكتب تعديل المقعد حسب التوفر النهائي.</p></div><span>{selectedSeats.length}/{seatPassengerCount}</span></div><div className="flight-seat-map">{seatRows.map((row) => <div className="flight-seat-row" key={row}><span>{row}</span>{seatColumns.slice(0,3).map((column) => { const seat = `${row}${column}`; const occupied = occupiedSeats.has(seat); return <button key={seat} type="button" disabled={occupied} className={`flight-seat ${occupied ? 'occupied' : ''} ${selectedSeats.includes(seat) ? 'selected' : ''}`} onClick={() => toggleSeat(seat)}>{seat}</button>; })}<span className="aisle" />{seatColumns.slice(3).map((column) => { const seat = `${row}${column}`; const occupied = occupiedSeats.has(seat); return <button key={seat} type="button" disabled={occupied} className={`flight-seat ${occupied ? 'occupied' : ''} ${selectedSeats.includes(seat) ? 'selected' : ''}`} onClick={() => toggleSeat(seat)}>{seat}</button>; })}</div>)}</div>{selectedSeats.length ? <div className="flight-seat-summary">{selectedSeats.map((seat) => <span key={seat}>{seat} · {formatMoney(seatPrice(seat))}</span>)}</div> : null}</section><div className="flight-price-breakdown"><div><span>سعر الركاب حسب الفئات العمرية</span><strong>{formatMoney((service.price + (option?.priceDelta || 0)) * flightPricingFactor)}</strong></div><div><span>فئة التذكرة والوجبات والمقاعد</span><strong>{formatMoney(flightUpgradeTotal)}</strong></div><div><span>التقدير قبل الخصم</span><strong>{formatMoney(estimate)}</strong></div></div></div> : <><section><h3>اختيار الباقة</h3>{service.options.length ? <div className="choice-card-grid">{service.options.map((item) => <label key={item.id} className={optionId === item.id ? 'selected' : ''}><input type="radio" name="service-option" checked={optionId === item.id} onChange={() => setOptionId(item.id)} /><div><strong>{item.label}</strong><span>{item.priceDelta ? `+ ${formatMoney(item.priceDelta)}` : 'مشمول بالسعر'}</span></div></label>)}</div> : <div className="inline-empty">لا توجد باقات إضافية؛ سيتم اعتماد الخيار الأساسي.</div>}</section></>}
            ...
            {step === 'confirm' ? <div className="booking-confirm screen-enter"><h3>راجع طلب الحجز</h3><dl><div><dt>الخدمة</dt><dd>{service.title}</dd></div><div><dt>المكتب</dt><dd>{service.office.name}</dd></div><div><dt>التاريخ</dt><dd>{formatDate(primaryDate)}{(service.type === 'hotel' || service.type === 'car' || tripType === 'round_trip') ? ` — ${formatDate(endDate)}` : ''}</dd></div><div><dt>العدد</dt><dd>{service.type === 'car' ? 'سيارة واحدة' : bookingQuantity}</dd></div>{service.type === 'flight' ? <><div><dt>فئات الركاب</dt><dd>{flightAdults} بالغ · {flightChildren} طفل · {flightInfants} رضيع</dd></div><div><dt>فئة التذكرة</dt><dd>{fareClass === 'light' ? 'Economy Light' : fareClass === 'standard' ? 'Economy Standard' : 'Economy Flex'}</dd></div><div><dt>الوجبة</dt><dd>{meal === 'standard' ? 'قياسية' : meal === 'vegan' ? 'نباتية' : 'فاخرة'}</dd></div><div><dt>المقاعد</dt><dd>{selectedSeats.length ? selectedSeats.join('، ') : 'تحدد لاحقًا'}</dd></div></> : null}{manifestRequired ? <div><dt>{service.type === 'car' ? 'السائق' : 'بيانات المسافرين'}</dt><dd>{travelers.length} {service.type === 'car' ? 'سائق' : 'مسافر'}</dd></div> : null}<div><dt>طريقة الدفع</dt><dd>{paymentMethod === 'Cash at Office' ? 'الدفع في المكتب' : paymentMethod}</dd></div><div><dt>السعر التقديري قبل الخصم</dt><dd>{formatMoney(estimate)}</dd></div></dl><div className="confirmation-note"><CheckCircle2 size={19} /><p>السعر النهائي والمقاعد والكوبون يعاد التحقق منها داخل قاعدة البيانات عند الإرسال. إنشاء الطلب لا يعني أن المكتب أكده.</p></div></div> : null}
            {error ? <div className="form-notice error" role="alert">{error}</div> : null}
          </div>
          <footer className="booking-dialog-footer">{currentStepIndex > 0 ? <button type="button" className="secondary-button" onClick={previous} disabled={submitting}><ArrowRight size={16} />السابق</button> : <span />}{step === 'confirm' ? <button type="button" className="primary-button" onClick={() => void submit()} disabled={submitting}>{submitting ? <Loader2 className="spin" size={17} /> : <CheckCircle2 size={17} />}إرسال طلب الحجز</button> : <button type="button" className="primary-button" onClick={next}><ArrowLeft size={16} />التالي</button>}</footer>
        </>}
      </section>
    </div>
  );
