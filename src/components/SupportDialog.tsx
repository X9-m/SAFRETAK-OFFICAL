...
export function SupportDialog({ booking, onClose, onSubmit }: SupportDialogProps) {
  ...
  return <div className="modal-backdrop modal-enter" onMouseDown={(event) => event.target === event.currentTarget && !busy && onClose()}><section ref={ref} className="support-dialog dialog-enter" role="dialog" aria-modal="true" aria-label="طلب المساعدة" tabIndex={-1}><header><div><CircleHelp size={20} /><h2>طلب مساعدة</h2></div><button type="button" className="icon-button" onClick={onClose} disabled={busy}><X size={18} /></button></header>{success ? <div className="booking-success"><CircleHelp size={45} /><h3>تم إرسال الطلب</h3><p>سيتابع فريق الدعم الطلب من حسابك.</p><button type="button" className="primary-button" onClick={onClose}>إغلاق</button></div> : <form className="dialog-form" onSubmit={submit}><label className="field-group"><span>عنوان الموضوع</span><input value={subject} onChange={(event) => setSubject(event.target.value.slice(0, 120))} maxLength={120} /></label><label className="field-group"><span>التفاصيل</span><textarea value={message} onChange={(event) => setMessage(event.target.value.slice(0, 1500))} maxLength={1500} placeholder="اشرح طلبك بالتفصيل" /><small className="field-hint">{message.length}/1500</small></label>{error ? <div className="form-notice error">{error}</div> : null}<button type="submit" className="primary-button" disabled={busy}>{busy ? <Loader2 className="spin" size={16} /> : <CircleHelp size={16} />}إرسال الطلب</button></form>}</section></div>;
}
