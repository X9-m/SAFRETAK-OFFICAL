...
export function HomePage({ services, offices, ads, favoriteIds, onSelectService, onFavorite, onSelectOffice, onBrowseServices }: HomePageProps) {
  ...
  const regionalRecommendations = useMemo(() => recommendRegionalServices(services, clientRegion, 6), [clientRegion, services]);
  ...
  const locateClient = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        ...
        try { window.localStorage.setItem(REGION_STORAGE_KEY, region.key); } catch { /* optional local preference */ }
      },
      () => setLocationState('unavailable'),
  ...
  useEffect(() => {
    ...
    try { cachedRegion = findRegionByKey(window.localStorage.getItem(REGION_STORAGE_KEY)); } catch { /* optional local preference */ }
    if (cachedRegion) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setActiveOfficeIndex((current) => (current + 1) % trendingOffices.length), 4200);
      ...
      <section className="trending-offices-section trending-offices-top" aria-labelledby="trending-offices-title">
        ...
        {trendingOffices.length ? (
          <div className="office-card-carousel" role="region" aria-label="المكاتب السياحية الرائجة">
            <div className={`office-card-carousel-track ${trendingOffices.length === 1 ? 'single-office' : ''}`} style={{ '--office-slide': activeOfficeIndex } as CSSProperties}>
              {trendingOffices.map((office, officeIndex) => (
                <button key={office.id} type="button" className={`trending-office-card ${officeIndex === activeOfficeIndex ? 'active' : ''}`} onClick={() => onSelectOffice(office)}>
                  <span className="trending-office-logo">{office.logoUrl ? <img src={office.logoUrl} alt="" loading="lazy" decoding="async" /> : <Building2 size={30} />}</span>
      ...
      {ads.length ? (
        <section className="ad-strip" aria-label="عروض المنصة">
          {ads.slice(0, 5).map((ad) => ad.link ? (
            <a key={ad.id} href={ad.link} className="ad-card" target="_blank" rel="noopener noreferrer"><img src={ad.imageUrl} alt={ad.titleAr} loading="lazy" decoding="async" /><strong>{ad.titleAr}</strong></a>
          ) : (
            <article key={ad.id} className="ad-card"><img src={ad.imageUrl} alt={ad.titleAr} loading="lazy" decoding="async" /><strong>{ad.titleAr}</strong></article>
          ))}
        </section>
      ) : null}
    </div>
